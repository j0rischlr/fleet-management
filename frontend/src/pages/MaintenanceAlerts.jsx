          ...
          {vehicleGroups.map((group, groupIndex) => (
            ...
            >
              <div className="p-6">
                ...
                {/* Actions */}
                {isAdmin() && (() => {
                  const hasInsurance = group.alerts.some(a => a.rule_name?.toLowerCase().includes('assurance'))
                  const hasInspection = group.alerts.some(a => a.rule_name?.toLowerCase().includes('contrôle technique'))
                  const hasOtherAlerts = group.alerts.some(a => !a.rule_name?.toLowerCase().includes('assurance') && !a.rule_name?.toLowerCase().includes('contrôle technique'))
                  return (
                    <div className="flex justify-end space-x-3 pt-4 mt-4 border-t border-gray-200">
                      <Link
                        to={`/dashboard/vehicles/${group.vehicle.id}`}
                      ...
                      {hasInsurance && (
                        <button
                          onClick={() => navigate(`/dashboard/vehicles/${group.vehicle.id}`, { state: { tab: 'insurance' } })}
                          className="inline-flex items-center px-3 py-2 border border-transparent rounded-md text-sm font-medium text-white bg-orange-500 hover:bg-orange-600"
                        >
                      ...
                      {hasInspection && (
                        <button
                          onClick={() => navigate(`/dashboard/vehicles/${group.vehicle.id}`, { state: { tab: 'inspection' } })}
                          className="inline-flex items-center px-3 py-2 border border-transparent rounded-md text-sm font-medium text-white bg-orange-500 hover:bg-orange-600"
                        >
                        ...
                        <button
                          onClick={() => navigate('/dashboard/maintenance', { 
                            state: { 
                              ...
                              alerts: group.alerts.filter(a => !a.rule_name?.toLowerCase().includes('assurance') && !a.rule_name?.toLowerCase().includes('contrôle technique')).map(a => ({
